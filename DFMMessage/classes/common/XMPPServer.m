       ...
       // NSLog(@"---message----%@",body);
        ...
        [DataOperation addUsingBlock:^(NSManagedObjectContext *context){
            ...
            if ([body hasPrefix:@"base64"]) {
                 NSLog(@"---message----  %@",[body substringWithRange:NSMakeRange(0, 11)]);
                if ([[body substringWithRange:NSMakeRange(0, 11)] isEqualToString:@"base64Audio"]) {
                    e.sendTyep=1;
                    NSData *data = [[body substringFromIndex:11] base64DecodedData];
                    AVAudioPlayer *player = [[AVAudioPlayer alloc] initWithData:data error:nil];
                    e.second = player.duration;
                    player = nil;
                    data = nil;

                }else if ([[body substringWithRange:NSMakeRange(0, 11)] isEqualToString:@"base64Image"]){
                    e.sendTyep=2;
                }
            }
        }];
